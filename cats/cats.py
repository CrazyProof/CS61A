...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    s = [para for para in paragraphs if select(para) == True]
    if k >= len(s): #这里需要一个条件判断,否则k太大的时候会报错，list index out of range
        return ''
    else:
        return s[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def para_contain_sub(para):
        s = split(lower(remove_punctuation(para)))
        for sub in subject: # 这里使用for word in s: 也可以
            if sub in s:
                return True
        return False
    return para_contain_sub
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    # 这里有一个问题，我们只需要计算typed中的word匹配准确率就够了。
    # 以typed中的word数量作分母，以match的word数量作分子
    # 在打字过程中，如果还没有打完，且以二者中长的作分母，就会出现准确率极低的情况，与实际不符
    satisfying = 0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    within_limit = [x for x in word_list if diff_function(typed_word, x, limit) <= limit]
    if within_limit == []:
        return typed_word
    s = min(within_limit, key = lambda w: diff_function(typed_word, w, limit)) # 这里用到了min([...], key = abs)这样一个key属性，可以使得在不改变原有list的情况下将diff计算出来的值进行比较，得到diff最小的x
    return s
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if limit < 0:
        return 1
    elif not typed: # 这里是空字符串，不是空列表，可以使用这种通用的技巧
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        return min(minimum_mewtations(typed, source[1:], limit-1) + 1, minimum_mewtations(typed[1:], source, limit-1) + 1, minimum_mewtations(typed[1:], source[1:], limit-1) + 1)
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        fastest = min([times[j][i] for j in player_indices])
        for k in player_indices:
            if fastest == times[k][i]:
                fast_word_list[k].append(words[i])
                break
