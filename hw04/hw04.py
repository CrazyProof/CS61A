def shuffle(s):
    ...
    left_list = s[:len(s) // 2]
    right_list = s[len(s) // 2:]
    new_list = []
    for i in range(len(s) // 2):
        new_list = new_list + [left_list[i]] + [right_list[i]]
    return new_list
...
def deep_map(f, s):
    ...
    for i in range(len(s)):
        if type(s[i]) == list:
            ...
            s[i] = f(s[i]) # 不能直接使用x = ··· 修改原数组对象，因为int是不可变对象
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    elif length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m))) and balanced(end(left(m))) and balanced(end(right(m))):
        return True
    else:
        return False
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        return True in [berry_finder(bs) for bs in branches(t)]
