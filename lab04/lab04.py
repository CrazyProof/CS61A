def divide(quotients, divisors):
    ...
    """
    return {k: [d for d in divisors if d % k == 0] for k in quotients}
...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            for k in range(1, amount // price + 1): # 可能会有很多种情况但是没有办法刚好用完钱，这些情况不会print
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    zero = make_city('zero', lat, lon)
    if distance(zero, city_a) >= distance(zero, city_b):
